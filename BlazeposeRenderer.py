...
class BlazeposeRenderer:
    ...
    def draw_landmarks(self, body, pd_only):
        if self.show_rot_rect or pd_only:
            cv2.polylines(self.frame, [np.array(body.rect_points)], True, (0,255,255), 2, cv2.LINE_AA)
        if self.show_landmarks and not pd_only:                
        ...
        if self.show_score and not pd_only:
            h, w = self.frame.shape[:2]
            cv2.putText(self.frame, f"Landmark score: {body.lm_score:.2f}", 
                        (20, h-60), 
                        cv2.FONT_HERSHEY_PLAIN, 2, (255,255,0), 2)

        if self.show_xyz and body.xyz_ref and not pd_only:
        ...
        if self.show_xyz_zone and body.xyz_ref and not pd_only:
    ...
    def draw(self, frame, body):
        if not self.pause:
            self.frame = frame
            if body:
                self.draw_landmarks(body, body.pd_kps is None)
            self.body = body
